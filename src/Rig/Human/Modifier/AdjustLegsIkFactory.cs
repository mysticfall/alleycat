...
{
    [Export] public Node3D? RightFootIkTarget { get; set; }

    [Export] public Node3D? LeftFootIkTarget { get; set; }
    ...
    protected override Eff<IEnv, IIkModifier> CreateService(
        ...
        from rightFoot in RightFootIkTarget.Require("Right foot target is not set.")
        from leftFoot in LeftFootIkTarget.Require("Left foot target is not set.")
