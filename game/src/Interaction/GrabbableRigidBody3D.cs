...
/// <summary>
/// Physical grabbable holder that delegates to child grab-point components and suspends rigid-body simulation while held.
/// </summary>
[GlobalClass]
public partial class GrabbableRigidBody3D : RigidBody3D, IGrabbable, IReleasableGrabbable
{
    ...
    {
        _physicsState ??= new PhysicsState(Freeze, FreezeMode, CollisionLayer, CollisionMask, LinearVelocity, AngularVelocity);
    ...
    {
        if (_physicsState is not PhysicsState state)
        {
            return;
        }
