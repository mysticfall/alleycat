...
const OUTPUT_ROOT := "vision-001-mirror-room-eye-follow"
...
func _run() -> void:
	if DisplayServer.get_name() == "headless":
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate must not run headless")
		return

	var mirror_room: Node3D = SceneUtils.instantiate_scene(MIRROR_ROOM_SCENE_PATH) as Node3D
	if mirror_room == null:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate failed to load %s" % MIRROR_ROOM_SCENE_PATH)
		return
	...
	if eye_subjects.is_empty():
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate failed to discover any generated-character eye subjects")
		return
	...
	print("VISION001_MIRROR_ROOM_EYE_FOLLOW_VISUAL_GATE_PASS artefact_root=game/temp/%s subjects=%d" % [OUTPUT_ROOT, eye_subjects.size()])
	quit(0)
...
func _capture_subject(mirror_room: Node3D, subject: Dictionary) -> void:
	...
	if eyes_behaviour == null or animation_tree == null or eye_origin == null:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate found incomplete subject '%s'" % label)
		return
...
func _create_face_camera(parent: Node, eye_origin: Node3D, label: String) -> Camera3D:
	...
	camera.name = "VISION001MirrorRoomEyeFollowFaceCamera_%s" % label
...
func _validate_camera(camera: Camera3D, eye_origin: Node3D) -> void:
	...
	if eye_to_camera_local.z >= 0.0:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate camera is not in front of the female face")
		return

	print(
		"VISION001_MIRROR_ROOM_EYE_CAMERA local_position=(%.4f, %.4f, %.4f) fov=%.2f" % [
...
func _apply_scenario(look_target: Node3D, eye_origin: Node3D, eyes_behaviour: Node, animation_tree: AnimationTree, scenario: Dictionary) -> void:
	...
	if local_target.z >= 0.0:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate scenario %s target is not in front of the face" % scenario["name"])
		return
...
func _assert_scenario_parameters(animation_tree: AnimationTree, scenario: Dictionary) -> void:
	...
	if absf(horizontal_blend - 1.0) > PARAMETER_TOLERANCE or absf(vertical_blend - 1.0) > PARAMETER_TOLERANCE:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate %s look blends disabled h=%.4f v=%.4f" % [scenario["name"], horizontal_blend, vertical_blend])
		return
	if not (scenario["blink"] as bool) and absf(horizontal_seek - (scenario["horizontal_seek"] as float)) > PARAMETER_TOLERANCE:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate %s horizontal seek %.4f did not match %.4f" % [scenario["name"], horizontal_seek, scenario["horizontal_seek"]])
		return
	if not (scenario["blink"] as bool) and absf(vertical_seek - (scenario["vertical_seek"] as float)) > PARAMETER_TOLERANCE:
		SceneUtils.fatal_error_and_quit("VISION-001 mirror-room eye-follow visual gate %s vertical seek %.4f did not match %.4f" % [scenario["name"], vertical_seek, scenario["vertical_seek"]])
		return

	print(
		"VISION001_MIRROR_ROOM_EYE_SCENARIO name=%s horizontal_seek=%.4f vertical_seek=%.4f" % [
			scenario["name"],
			horizontal_seek,
			vertical_seek,
		]
