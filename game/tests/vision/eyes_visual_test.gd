...
func _run() -> void:
	if DisplayServer.get_name() == "headless":
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner must not run headless because screenshots would be invalid")
		return

	var photobooth: Photobooth = SceneUtils.instantiate_scene(TEST_SCENE_PATH) as Photobooth
	if photobooth == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: failed to load test scene: %s" % TEST_SCENE_PATH)
		return
	...
	if female == null or viewpoint == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: failed to resolve female subject or viewpoint")
		return
	...
	print("VISION001_EYES_VISUAL_GATE_PASS artefact_root=%s" % OUTPUT_ROOT)
...
func _validate_camera_rig(photobooth: Photobooth) -> void:
	...
	if camera_rig == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: missing face CameraRig")
		return

	if camera_rig.camera == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: face CameraRig has no camera")
		return

	print("VISION001_EYES_CAMERA rig=CameraRig camera=%s projection=%s" % [camera_rig.camera.name, camera_rig.camera.projection])
...
func _validate_target_marker_orientation(viewpoint: Node3D, look_target: Node3D) -> void:
	...
	if local_target.z >= 0.0:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: neutral look target is not in front of the face")
		return

	print("VISION001_EYES_MARKER neutral_local=(%.4f, %.4f, %.4f)" % [local_target.x, local_target.y, local_target.z])
...
func _assert_scenario_parameters(animation_tree: AnimationTree, scenario: Dictionary) -> void:
	...
	if not (scenario["blink"] as bool) and absf(horizontal_seek - expected_horizontal) > tolerance:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: %s horizontal seek %.4f did not match %.4f" % [scenario["name"], horizontal_seek, expected_horizontal])
		return
	if not (scenario["blink"] as bool) and absf(vertical_seek - expected_vertical) > tolerance:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: %s vertical seek %.4f did not match %.4f" % [scenario["name"], vertical_seek, expected_vertical])
		return
	print(
		"VISION001_EYES_SCENARIO name=%s horizontal_seek=%.4f vertical_seek=%.4f" % [
			scenario["name"],
			horizontal_seek,
			vertical_seek,
		]
...
func _attach_animation_tree(female: Node3D) -> AnimationTree:
	...
	if animation_tree == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: AnimationTree scene root must be AnimationTree")
		return null
...
func _attach_eyes_behaviour(animation_tree: AnimationTree, viewpoint: Node3D, look_target: Node3D) -> Node:
	...
	if eyes_script == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: failed to load EyesBehaviour script")
		return null
...
func _get_default_property_value(instance: Object, property_name: StringName) -> Variant:
	...
	if script == null:
		SceneUtils.fatal_error_and_quit("VISION-001 eyes runner: failed to resolve script defaults for %s" % property_name)
		return null
