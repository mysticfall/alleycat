---
id: CHAR-003
title: Character Physical Response
legacy_id: BODY-002
---
...
8. `DynamicPhysicalRig`, physical collision proxy generation, and physical-response
   slices shall not depend on IK. The IK/pose/target pipeline may consume
   `Rigging.Physics`-owned `BodyColliderProfile` data, generated proxies, contact, and actuator
   information. Existing specialised hand-pose and hand integration code is outside this
   proxy-generation dependency boundary unless or until it is separately cleaned up.
...
- Child spec boundaries for impact reaction, stagger, grab constraints, and ragdoll
  bridge.
- RIG-001 boundary: RIG-001 handles collision/proxy/hand-obstruction decisions.
  Full reactive target pipeline details are future character-physical-response work.
...
- Optional response-mode expansion beyond the normative contracts linked below.
- Cleanup of existing specialised hand-pose and hand integration code outside the
...
8. Technical Requirement 8 (`DynamicPhysicalRig`/IK dependency direction) is validated
   by confirming `DynamicPhysicalRig`, physical collision proxy generation, and
   physical-response slices do not depend on IK; IK may consume `Rigging.Physics`-owned
   `BodyColliderProfile` data, generated proxies, contact, and actuator data.
...
- [Character Physical Response Design Notes](design-notes.md)
...
- [Character Skeleton Profile](../001-character-skeleton/index.md)
