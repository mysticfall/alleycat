---
id: RIG-001
title: Physical Collision
legacy_id: BODY-003
---

# Physical Collision

> **Related spec:** [CHAR-003: Character Physical Response](../../character/003-physical-response/index.md)
...
4. Authored body colliders should be reusable for non-rigging consumers, such as
    future IK queries, without coupling those consumers to rigging runtime nodes.
5. Character authors should be able to select the body collider profile that
   matches a character's shape from character-level authoring surfaces, without
   expanding inherited physical-rig installer internals.

## Technical Requirements

Physical rig API types shall live in `AlleyCat.Rigging.Physics`; `PhysicalInteractionReceipt` shall live in
`AlleyCat.Interaction.Physical`.
...
4. Character role installation may provide an optional character-level
   `BodyColliderProfile` through typed install context. `DynamicPhysicalRig`
   remains responsible for consuming the resulting profile; context transport
   shall not be replaced by a generic service bag or by actual scenes reaching
   into child dynamic physical rig installer internals.
5. Dynamic physical rig installation shall resolve collider profile precedence
   as follows: root role installer context profile first, child installer default
   second, and copied template rig profile third. The selected profile must still
   match the target character shape.
6. For each source `CollisionShape3D`, the implementation shall resolve the
...
7. Non-finger source shapes resolved from the profile shall produce generated
...
8. For each finger bone on the target skeleton, the rig shall generate at most
   ...
   generated primitive finger geometry rather than copied:
    ...
    e. Count and expose generated finger proxies separately from profile-driven
       proxies.
9. Generated proxy bodies shall follow animated/posed bones solely through
    normal `BoneAttachment3D` parent-child transform inheritance. Proxy generation
   shall not expose a separate proxy-body synchronisation API for this topology.
10. The rig shall clear and rebuild deterministically on each initialisation.
11. Reference IK target nodes for the head, hands, and feet shall not carry
...
12. Adjacent-bone bidirectional collision exceptions shall be applied between
    each generated non-finger body and its parent bone's generated body to
    prevent self-collision.
13. Same-side finger self-collision exceptions shall be applied per rig instance:
...
14. Collision layers shall be configured consistently for generated proxy bodies.
...
15. Hand actuators shall remain `AnimatableBody3D` nodes driven from
...
16. `CharacterIK` shall expose an optional exported `PhysicalRig` property of
...
17. `CharacterIK.UpdatePhysicalActuators()` shall invoke a pre-hand-actuator
...
18. Coupling direction remains IK → `Rigging.Physics` only. `DynamicPhysicalRig` shall not
    reference or depend on any IK nodes, components, or state. `Rigging.Physics` collision-shape
    data, including `BodyColliderProfile` data and generated proxies, flows to IK through the
    public API; IK remains the consumer and `Rigging.Physics` remains the independent provider.
19. Hand-to-dynamic-body interaction shall use explicit capped channels through
...
20. Dynamic bodies eligible for explicit hand interaction must have collision
    layer 2 and belong to group `hand_dynamic_interaction_body`.
21. The hand interaction controller shall be configurable with strength-style
    parameters (thresholds, gains, and caps) rather than prop-specific rules.
22. Automated test coverage shall verify rig generation, hand actuator setup,
    collision-layer contract, and explicit hand-to-dynamic-body force transfer.
23. Collision responses shall not override or conflict with physics-based
    head/hand target driving.
24. Runtime hand-target movement collision shapes generated for IK shall reuse
    ...
    runtime/generated nodes, not authored primitive target shapes, and dependency
      direction remains IK → `Rigging.Physics`-owned `BodyColliderProfile` data and generated
     proxies, with no Rigging → IK dependency.
...
- Character-level role installer configuration for selecting the body collider
  profile consumed by dynamic physical rig installation.
...
- IK consumers (e.g. `PlayerVRIK`) consuming an explicit `CharacterIK.PhysicalRig`
  reference for generated collision-proxy integration, with no per-frame skeleton child discovery.
  Dependency direction remains IK → `Rigging.Physics`-owned `BodyColliderProfile` data and
  generated proxies, with no Rigging → IK coupling. IK-owned visible hand blocking belongs to
  the IK subsystem rather than to generated body proxies.
...
7. Character scenes can configure a character-specific collider profile on the
   root role installer without expanding inherited child dynamic physical rig
   installer internals.
8. Tests or scene review verify dynamic physical rig installation applies profile
   precedence as root context profile first, child installer default second, and
   copied template rig profile third.
9. Code review confirms character-level profile transport uses typed install
   context; no generic service bag or actual-scene child installer reach-in is
   introduced.
10. For each source shape, the implementation walks to the closest ancestor
...
11. If `BoneName` is empty or does not resolve against the target skeleton,
...
12. Non-finger proxy bodies use descriptor `Shape3D` resources below generated
   ...
   `LocalTransform`.
13. Tests verify that authored non-finger proxy placement does not use the body
...
14. Tests verify that proxies move with animated/posed bones at runtime through
...
15. Generated shape transforms preserve authored collider pose in attachment-local
...
16. The rig clears and regenerates deterministically on each initialisation.
17. Reference head, hand, and foot IK targets have no direct primitive
    `CollisionShape3D` children; their anatomical collision shape data is
    sourced from `BodyColliderProfile` and generated proxies.
18. Adjacent-bone collision exceptions are applied bidirectionally for
    non-finger profile-driven bodies.
19. For each finger bone, the rig generates at most one capsule proxy body from
...
20. Finger proxy count is exposed separately from total proxy count.
21. Same-side finger self-collision exceptions are applied per rig instance:
...
22. Collision layers are correctly configured for generated proxies:
...
23. Hand actuators use `AnimatableBody3D` with physics-timed position updates
    ...
    exceptions.
24. Hand-to-dynamic-body interaction uses explicit capped impact and sustained
    push channels, with profile-backed query shapes when direct primitive hand
    target shapes are absent.
25. Dynamic bodies receive explicit hand interaction only when they have
    collision layer 2 and belong to group `hand_dynamic_interaction_body`.
26. Impact and sustained channels are governed by configurable parameters.
27. Head collision remains out of scope; no requirement mandates head collision
    response that could conflict with head target driving.
28. Automated tests verify the core contracts for rig generation, hand actuator
     setup, collision layers, and explicit force transfer.
29. Automated tests verify that a shapeless authored hand target gains runtime
...
30. Tests verify `DynamicPhysicalRig.GetGeneratedFingerProxyCollisionShapesForHand`
...
31. Tests verify mirrored finger proxy collision shapes exist under each hand IK
     target node in `PlayerVRIK`, and hand-actuator movement collides against the
     same finger proxies that body animation uses.
32. Tests verify `CharacterIK.UpdatePhysicalActuators()` invokes the pre-hand-actuator
...
33. Code review confirms `DynamicPhysicalRig` contains no references to IK nodes,
    components, or state, preserving the IK → `Rigging.Physics` coupling direction.
