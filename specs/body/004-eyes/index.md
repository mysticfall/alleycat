...
reference model uses a combined mesh for both eyes.
...
| 10 | Technical         | Implementation does not depend on perception or sight AI systems. |
| 11 | Technical         | Tests verify the eyes component is discoverable via `IEyesHolder`. |
| 12 | Technical         | Tests verify that TimeSeek seek position 0.5 corresponds to neutral eye |
|    |                   | position (forward-facing). |
| 13 | Technical         | Tests verify that blink playback uses an `AnimationNodeOneShot` request, |
|    |                   | while horizontal and vertical look remain `AnimationNodeTimeSeek` nodes. |
| 14 | Technical         | Mirror-room tests verify that look blend overrides remain enabled at runtime. |
