...
- Reference subjects by full ID rather than by name in prompt-facing lore: subject-bound entries carry the subject's
  full ID (`[type]:[id]`, with types `char`, `loc`, and `item`) as the frontmatter `title`, which the lore formatter
  renders as a Markdown heading in prompts, and body prose uses the full ID where the name would appear while
  pronouns and purely descriptive references remain natural.
...
- Canonical subject-bound entries follow the same convention, with full-ID titles, and state the subject's name
  once as an explicit fact.
