...
public sealed class MirrorRoomIntegrationTests
{
    ...
    private static void AssertAllyVoiceAndMind()
    {
        ...
        string allySceneText = ReadResourceText(AllyScenePath);

        Assert.DoesNotContain("AIVoice.cs", allySceneText, StringComparison.Ordinal);
        Assert.DoesNotContain("AgenticMind.cs", allySceneText, StringComparison.Ordinal);
        Assert.DoesNotContain("[node name=\"Voice\"", allySceneText, StringComparison.Ordinal);
        Assert.DoesNotContain("[node name=\"Mind\"", allySceneText, StringComparison.Ordinal);
